...
export class WinnersEffects {
    ...
    loadWinners$ = createEffect(() =>
        this.action$.pipe(
            ...
            mergeMap(([, page, limit, sort, order]) =>
        ...
        this.action$.pipe(
            ...
            mergeMap(action =>
                this.service.createWinner(action.winner).pipe(
                    withLatestFrom(
                        this.garageService.getCarById(action.winner.id),
                    ),
                    mergeMap(([data, car]) =>
                        of(createWinnerSuccess({ winner: data, car })).pipe(
                            catchError(() => EMPTY),
                        ),
                    ),
                ),
    ...
    updateWinner$ = createEffect(() =>
        this.action$.pipe(
            ...
            mergeMap(action =>
                this.service
                    .updateWinner(action.winnerID, action.winnerData)
                    .pipe(
